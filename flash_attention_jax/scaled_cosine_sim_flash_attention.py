...
def _query_chunk_flash_attention(chunk_idx, q, k, v, key_mask):
    ...
    def chunk_scanner(carries, _):
        ...
        attn_weights = q @ k_chunk.transpose() # the output of this will range from [-2 * scale, 0], and the row sums are now bounded by key/value sequence length - you can also shift this more if you wish to tailor the normalization constant (in the case of extreme sequence lengths)
...
@jit
def l2norm(t, scale = 1.0):
    t *= scale / (jnp.linalg.norm(t) + EPSILON)
    return t 

@jit
def scaled_cosine_sim_flash_attention(q, k, v, key_mask, scales: Optional = None):
    if scales is not None:
        normalize = lambda x: l2norm(x, scale = jnp.sqrt(scales))
    else:
        normalize = l2norm
